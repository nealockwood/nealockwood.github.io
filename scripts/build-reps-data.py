...
Shocks/BS controls come from jk_source_old.csv (paulbousquet/GBMPSurprise), stored as raw EVENTS
and aggregated (main/month-last/-first/-sum) + standshock'd in the browser; the gaps (df/dp/dg/du)
come from the Stata consensus runs. IRF uses the lpdecomp methodology (paulbousquet/lpdecomp).
"""
...
def assemble():
    ...
    # --------------- jk_source_old EVENTS (in-browser aggregation + standshock) ---------------
    JK="https://raw.githubusercontent.com/paulbousquet/GBMPSurprise/main/jk_source_old.csv"
    jk=pd.read_csv(io.BytesIO(urllib.request.urlopen(urllib.request.Request(JK,headers={"User-Agent":"x"}),timeout=120).read()))
    jk["rd"]=pd.to_datetime(jk["Date"],format="%m/%d/%Y",errors="coerce")
    jk=jk.dropna(subset=["rd"]); jk["ym"]=jk["rd"].dt.strftime("%Y-%m")
    for c in ["MP1","FF4","ED1","ED2","ED3","ED4","NFP_SURP","NFP_12M","SP500_3M","SLOPE_3M","BCOM_3M","TR_SKEW","Unscheduled","main"]:
        jk[c]=pd.to_numeric(jk[c],errors="coerce")
    msub=jk[jk["main"]==1].dropna(subset=["ED1","ED2","ED3","ED4"])
    Xe=msub[["ED1","ED2","ED3","ED4"]].values; sde=Xe.std(0); sde[sde==0]=1.0
    Xes=(Xe-Xe.mean(0))/sde; _,_,vte=np.linalg.svd(Xes,full_matrices=False); load=vte[0]
    bm=np.polyfit(Xes@load, msub["ED4"].values,1)[0]
    def mps_of(row):
        e=[row["ED1"],row["ED2"],row["ED3"],row["ED4"]]
        if any(pd.isna(v) for v in e): return None
        z=(np.array(e,float)-Xe.mean(0))/sde; return rnd(0.01*float(z@load)/bm)
    events=[{"date":r["rd"].strftime("%Y-%m-%d"),"month":r["ym"],
             "scheduled":int(r["Unscheduled"]==0) if pd.notna(r["Unscheduled"]) else None,
             "main":int(r["main"]) if pd.notna(r["main"]) else None,
             "mp1":rnd(r["MP1"]),"ff4":rnd(r["FF4"]),"ed4":rnd(r["ED4"]),"mps":mps_of(r),
             "rtn":{"nfp_surp":rnd(r["NFP_SURP"]),"nfp_12m":rnd(r["NFP_12M"]),"sp500_3m":rnd(r["SP500_3M"]),
                    "slope_3m":rnd(r["SLOPE_3M"]),"bcom_3m":rnd(r["BCOM_3M"]),"tr_skew":rnd(r["TR_SKEW"])}}
            for _,r in jk.iterrows()]

    payload={"meta":{"built_at":datetime.now(timezone.utc).isoformat(timespec="seconds"),"repo":REPO,
        "generator":"Stata romer/monthlydisag.do (gaps, per series) + stablediag.do (panel); shocks from jk_source_old events (in-browser aggregation + standshock); IRF via lpdecomp methodology",
        ...
        "aggregations":["main","month-last","month-first","month-sum"],
        "monthly":monthly,"panel":panel,"events":events}
